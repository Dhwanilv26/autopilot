...
import tomli
from autopilot.utils.errors import ConfigError
...
def get_config_dir() -> Path:
    # global config path dir -> ~/Library/Application Support/autopilot/config.toml (in macos) platform dirs is used to get global file access, the path is different on windows/linux
    return Path(user_config_dir("autopilot"))


def get_data_dir() -> Path:
    return Path(user_data_dir("autopilot"))
...
def _get_project_config(cwd: Path) -> Path | None:
    # project config path -> nodebase/.autopilot/config.toml
    # specific to the current project only
    current = cwd.resolve()
    agent_dir = current/'.autopilot'
